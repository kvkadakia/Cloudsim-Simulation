...
import org.cloudbus.cloudsim.{Cloudlet, CloudletSchedulerSpaceShared, Datacenter, DatacenterBroker, DatacenterCharacteristics, Host, Log, Pe, Storage, UtilizationModel, UtilizationModelFull, Vm, VmAllocationPolicySimple, VmSchedulerTimeShared}

import scala.collection.JavaConverters._

//Space shared policy for the vms
//Time shared policy for the host
//VM space shared policy means that the vm shares the space to run cloudlets on them
//Host time shared policy is for assigning the PEs to the VMs

//2 vms and 3 cloudlets

object CloudSimExample2 {
  ...
  def main(args: Array[String]): Unit = {
      ...
      // create 3 VM'a
      val vm : Vm = new Vm(vmid, brokerId, mips, pesNumber, ram, bw, size, vmm, new CloudletSchedulerSpaceShared)

      val vm1 : Vm = new Vm(vmid1, brokerId, mips, pesNumber, ram, bw, size, vmm, new CloudletSchedulerSpaceShared)

      val vm2 : Vm = new Vm(vmid2, brokerId, mips, pesNumber, ram, bw, size, vmm, new CloudletSchedulerSpaceShared)
